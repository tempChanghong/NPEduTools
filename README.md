...
**当前发布候选：InDev 20260920。** [发版说明](docs/releases/InDev-20260920.md)汇总本版功能、安装方式和已知限制；GitHub Release 在审核期间保持 Draft / Pre-release。下文按开发阶段保留的验收状态需结合较新的报告阅读：本机模式正常往返和取消授权已验证，目标大屏整课及实际登录启动仍待现场测试。
...
面向 Windows 教室大屏的本地软件集成与控制层。主程序提供常驻桌面侧边栏与“概览／快捷启动／设置”管理窗口，已集成 PowerPoint 触摸翻页。侧边栏默认位于右侧偏下，可拖动顶部品牌图标调整高度；课堂工具、我的快捷和设置各有独立入口，展开后可开启、暂停或停止辅助。运行 `./scripts/start-app.ps1` 查看实际窗口，界面与生命周期见 [主界面设计](docs/MAIN-WINDOW-DESIGN.md)。
...
窗口会按需启动 Host。标题栏关闭、最小化或“快捷面板”隐藏主窗口，屏幕侧边入口、触摸辅助与后台继续运行；托盘和再次打开程序也可唤回同一窗口。主窗口“退出 NPEduTools”或托盘“停止后台并退出”释放触摸监听与探测进程后完整退出，不关闭 ClassIsland、PowerPoint 或文稿。设置支持 [NPEduTools 登录启动、静默贴边入口和自动开启触摸辅助](docs/APP-STARTUP.md)。场景执行尚未实现；ClassIsland 管理员重启与自启动由独立管理区显式操作。
...
首页提供触摸翻页开关、暂停／继续及 ClassIsland 启动入口。首次运行默认关闭触摸辅助，可在设置中选择启动时自动开启。登录自启动默认关闭；启用后默认仅显示贴边入口，手动打开仍显示主窗口。后台断开时会清除可用状态并重新连接，不自动重新启用已因后台退出而停止的辅助。独立辅助版和主程序共用互斥检查，不能同时运行两套监听。
