...
窗口会按需启动 Host。标题栏关闭、最小化或“快捷面板”隐藏主窗口，屏幕侧边入口、触摸辅助与后台继续运行；托盘和再次打开程序也可唤回同一窗口。主窗口“退出 NPEduTools”或托盘“停止后台并退出”释放触摸监听与探测进程后完整退出，不关闭 ClassIsland、PowerPoint 或文稿。本阶段尚未实现场景执行、关闭外部软件或开机启动。
