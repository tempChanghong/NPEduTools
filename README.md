...
面向 Windows 教室大屏的本地软件集成与控制层。主程序已采用“常用／设置”紧凑界面，并集成 PowerPoint 触摸翻页、托盘和单实例唤回。运行 `./scripts/start-app.ps1` 查看实际窗口，界面与生命周期见 [主界面设计](docs/MAIN-WINDOW-DESIGN.md)。
...
窗口会按需启动 Host。关闭窗口或“收起到托盘”隐藏主窗口，触摸辅助与后台继续运行；点击托盘图标或再次打开程序即可唤回同一窗口。“停止后台并退出”释放触摸监听与探测进程后退出，不关闭 ClassIsland、PowerPoint 或文稿。本阶段尚未实现场景执行、关闭外部软件或开机启动。
...
“设置”中可选择触摸兼容模式、配置 ClassIsland 程序位置，并展开课程、课表和连接详情。ClassIsland 未运行不影响触摸辅助；目标大屏现场试用暂缓。
