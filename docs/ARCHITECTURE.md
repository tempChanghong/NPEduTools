...
> 管理员自启动实施更新（2026-09-06）：已增加 App → 独立 UAC 组件的任务管理及管理员启动／重启，兼容 StartUpAsAdmin 的 `ClassIsland.AdminStartup` 任务。普通 Host 不持有管理员权限；重启只请求正常退出。已完成只读与取消分支验证，用户已实测 UAC 授权后创建任务成功并提供权限状态截图；其余验收范围见 [管理员自启动说明](CLASSISLAND-ADMIN-STARTUP.md)。下文关于“管理员自启动暂缓”的早期优先级由本次更新替代。

> 主界面实施更新（2026-09-06）：已整合 PowerPoint 触摸辅助控制、右侧偏下的贴边快捷入口、常用／设置页面、托盘和单实例唤回，见 [主界面设计](MAIN-WINDOW-DESIGN.md)。标题栏关闭或最小化隐藏主窗口并保留贴边入口；“退出 NPEduTools”才完整退出。快捷面板与主窗口复用同一后台状态及操作，托盘作为补充入口。下文早期的 UI 退出语义以本次实现为准，通用场景引擎仍未实施。
