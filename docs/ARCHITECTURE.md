...
实施更新：M0 使用 CLI 验证进程边界；M1 已建立最小 WPF 窗口、常驻只读监听、自动重连与完整快照订阅。由于 ClassIsland IPC 连接不可取消且属性读取同步阻塞，单次查询和常驻监听均在 Host 创建的隔离进程中执行；Host 自有通信不依赖该第三方 IPC 库。启动外部软件、持久化及场景执行仍未实现。
