...
> 状态：初稿，供讨论与实施拆分使用。
> 日期：2026-09-06。
> 本文描述目标架构。现已建立 M0 只读查询原型，实际完成范围见 [M0 验证记录](M0-VALIDATION.md)，不能将目标设计视为全部已实现。

实施更新：M0 使用 CLI 验证 App/Host 进程边界，尚未建立 WPF 界面。由于 ClassIsland IPC 连接不可取消且属性读取同步阻塞，已提前引入一次性只读探测进程；Host 自有通信不依赖该第三方 IPC 库。
...
本文仅基于仓库现有资料进行架构设计，尚未联网核实参考项目的最新接口，也未运行真实软件兼容测试。引用设计思想不等于已经验证外部项目的具体实现；复用代码或分发第三方组件时需单独核对其许可条件。
