...
本文记录 `codex/npep-n1-device` 阶段的独立 CLI 用法。2026-09-21 已在后续 `codex/npep-n1-ui` 分支接入 WPF 设置页及 Host 后台连接管理，产品使用方式见 [UI 与生命周期记录](NPEP-N1-UI-REPORT.md)。未配对不自动联网；完成现场确认且未暂停后，由 Host 管理连接。不要用独立 CLI 争用正式 Host 的凭据目录。
...
2026-09-21 增加 [断线与独立进程恢复验收](NPEP-N1-RESILIENCE-REPORT.md)：此程序还会执行真实 65 秒断线窗口、丢回执和三次独立测试进程恢复，整体需数分钟；本轮不传 `--pipe` 时 26 项通过。内部 `--runtime-worker` 由父进程调用，不用于人工配对。

设置页、后台生命周期和学校管理页的后续进展见上述新报告；真实大屏、生产运维与更长时间断网仍须按 [现场验收表](NPEP-N1-FIELD-ACCEPTANCE.md) 验收。N2 通知和 N3 模式切换尚未开放。
