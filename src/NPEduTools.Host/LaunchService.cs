...
public sealed class LaunchService : IAsyncDisposable
{
    ...
    public async Task<HostResponse> HandleAsync(HostRequest request, CancellationToken token)
    {
        ...
        try
        {
            ...
            if (request.Capability == "classisland.start" && document.Executions.Find(x => x.RequestId == request.RequestId) is { } previous)
            ...
            var pending = new LaunchExecution(request.RequestId, executable, now, now, "Running", null, "正在检查并启动 ClassIsland…");
            retained.Add(pending);
            _store.Save(document with { Executions = retained });
            _active = Task.Run(() => ExecuteAsync(pending));
    ...
    private async Task ExecuteAsync(LaunchExecution execution)
    {
