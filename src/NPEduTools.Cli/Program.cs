...
{
    Console.WriteLine("NPEduTools.Cli <ping|status> [--pipe NAME] [--timeout-ms 3000] [--observe-ms 0]\nOutputs JSON. Exit: 0 success, 2 invalid arguments, 3 Host unavailable, 4 query failed, 130 cancelled.");
...
if (args[0] is not ("ping" or "status") ||
...
var request = new HostRequest(Protocol.Version, Guid.NewGuid(), args[0] == "ping" ? "host.ping" : "classisland.status", timeout, observe);
