...
{
    Console.WriteLine("NPEduTools.Cli <ping|status|schedule|school-clock|day-plan|watch|stop> [--pipe NAME] [--timeout-ms 3000] [--observe-ms 0] [--date yyyy-MM-dd]\nday-plan requires --date (school calendar). watch streams JSON until Ctrl+C. Exit: 0 success, 2 invalid arguments, 3 Host unavailable, 4 query failed, 130 cancelled.");
    return args.Length == 0 ? 2 : 0;
}
var options = new Dictionary<string, string>();
for (int i = 1; i < args.Length; i += 2)
{
    if (i + 1 >= args.Length || args[i] is not ("--pipe" or "--timeout-ms" or "--observe-ms" or "--date") ||
        !options.TryAdd(args[i], args[i + 1])) return 2;
}
if (args[0] is not ("ping" or "status" or "schedule" or "school-clock" or "day-plan" or "watch" or "stop") ||
...
DateOnly? schoolDate = null;
if (options.TryGetValue("--date", out var dateText))
{
    if (!DateOnly.TryParseExact(dateText, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var date)) return 2;
    schoolDate = date;
}
string capability = args[0] switch { "ping" => "host.ping", "stop" => "host.stop", "watch" => "classisland.watch", "school-clock" => "classisland.school-clock", "day-plan" => "classisland.day-plan", "schedule" => "classisland.schedule", _ => "classisland.status" };
var request = new HostRequest(Protocol.Version, Guid.NewGuid(), capability, timeout, observe, SchoolDate: schoolDate);
