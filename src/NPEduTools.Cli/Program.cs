...
{
    Console.WriteLine("NPEduTools.Cli <ping|status|schedule|school-clock|watch|stop> [--pipe NAME] [--timeout-ms 3000] [--observe-ms 0]\nwatch streams JSON until Ctrl+C. Exit: 0 success, 2 invalid arguments, 3 Host unavailable, 4 query failed, 130 cancelled.");
...
if (args[0] is not ("ping" or "status" or "schedule" or "school-clock" or "watch" or "stop") ||
...
string capability = args[0] switch { "ping" => "host.ping", "stop" => "host.stop", "watch" => "classisland.watch", "school-clock" => "classisland.school-clock", "schedule" => "classisland.schedule", _ => "classisland.status" };
