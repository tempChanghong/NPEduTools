...
public sealed record ClassroomModeCommand(string Target, bool SwitchRunning = false, bool ExamWorkSaved = false);
public sealed record ClassroomRuntimeIntent(string Target, ClassroomStartupSnapshot Startup, DateTimeOffset StartupCheckedAt, string Step = "Prepare");
...
public sealed record ClassroomModeState(long Revision = 0, string Mode = "Unconfigured", string Phase = "Idle",
    ...
    Guid[]? RecentRequests = null, ClassroomRuntimeIntent? Runtime = null)
{
    public bool Busy => Phase is "Checking" or "Switching" or "Running";
}
