...
if (args.Length > 2 || args.Length == 1 && args[0] != "hello" || args.Length == 2 && args[0] != "day") return 2;
...
try
{
    ...
    string json = await (args.Length == 2 ? provider.CreateIpcProxy<IRecordingBridgeCalendar>(client.PeerProxy!).GetDayAsync(args[1]) :
        args.Length == 1 ? service.GetHelloAsync() : service.GetSnapshotAsync()).WaitAsync(TimeSpan.FromSeconds(3));
